...
import static net.sf.smc.model.SmcElement.DEFAULT_STATE;
 ...
 #include &lt;%include header file&gt;
    ...
    public void visit(final SmcFSM fsm)
    {
        ...
        for (SmcMap map: fsm.getMaps())
        {
            ...
            for (SmcState state: map.getStates())
            {
                for (SmcTransition trans: transList)
                {
                    // Output transition definition if:
                    // 1. It is *not* the special Default
                    //    transition.
                    // 2. It is *not* defined in this state.
                    // 3. It is *not* defined in Default state.
                    //
                    // Note: those transitions defined only in
                    //       the Default state will be generated
                    //       when visiting the map.
                    if (!trans.isDefaultTransition() &&
                        !state.hasTransition(trans) &&
                        !defaultState.hasTransition(trans))
                    {
                ...
                // Output the default transition only if the
                // state does *not* have an explicit Default
                // transition.
                if (!state.hasDefaultTransition())
                {
                    mTarget.print("#define ");
                    mTarget.print(mapName);
                    mTarget.print("_");
            ...
            for (SmcTransition trans: transList)
            {
                // Output transition define if:
                // 1. It is *not* the special Default transition.
                // 2. It is *not* defined in Default state.
                //
                // Note: thoese transitions defined only in the
                //       Default state will be generated when
                //       visiting the map.
                if (!trans.isDefaultTransition() &&
                    !defaultState.hasTransition(trans))
                {
    ...
    public void visit(final SmcMap map)
    {
        ...
        if (map.hasDefaultState())
        {
                     ...
                     defaultState.getTransitions())
            {
                ...
                // Now that the Default state transition is
                // defined, you can have state transitions use
                // it.
                for (SmcState state: map.getStates())
                {
                    stateName = state.getInstanceName();

                    // If the transition is *not* defined in the
                    // state, then define the transition name
                    // to be a default transition alias.
                    if (!state.hasTransition(trans))
                    {
                        mTarget.print("#define ");
                        mTarget.print(mapName);
                        mTarget.print("_");
    ...
    public void visit(final SmcTransition transition)
    {
        ...
        else if (mGuardIndex > 0)
        {
            ...
            mTarget.print("        ");
            if (!stateName.equals(DEFAULT_STATE))
            {
                mTarget.print(mapName);
                mTarget.print("_DefaultState_");
            }
    ...
    public void visit(final SmcGuard guard)
    {
              ...
              !loopbackFlag))
        {
            if (mDebugLevel >= DEBUG_LEVEL_1)
            {
                ...
                mTarget.println(
                    "EXIT_STATE(fsm, getState(fsm))\\n\");");
                mTarget.print(indent2);
                mTarget.println("}");
            }

            mTarget.print(indent2);
            mTarget.println("EXIT_STATE(fsm, getState(fsm));");

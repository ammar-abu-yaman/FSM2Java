 ...
 #include <i>%include header file</i>
    ...
    public void visit(SmcFSM fsm)
    {
            ...
            )
        {
            ...
            for (stateIt = map.getStates().iterator();
                 ...
                 ++index)
            {
                // v. 7.4.0: set state name regardless of whether
                // exceptions are turned off since the state
                // name is needed for state machine debug
                // tracing.
                state = stateIt.next();
                mTarget.print(mIndent);
    ...
    public void visit(SmcGuard guard)
    {
        ...
        if (mDebugLevel >= DEBUG_LEVEL_0)
        {
            ...
            if (!mNoCatchFlag && !actions.isEmpty())
            {
                indent3 = indent2 + "    ";
            }
            else
        ...
        if (transType == TransType.TRANS_SET &&
            (!actions.isEmpty() || loopbackFlag == false))
        {
            mTarget.print(indent3);
        ...
        else if (transType == TransType.TRANS_PUSH)
        {
            ...
            // state was done.
            if (loopbackFlag == false || !actions.isEmpty())
            {
                mTarget.print(indent3);
                mTarget.print("context.setState(");
                mTarget.print(fqEndStateName);
        ...
        // v. 2.2.0: Check if the user has turned off this
        // feature first.
        if (!actions.isEmpty() && !mNoCatchFlag)
        {
            mTarget.print(indent2);
            mTarget.println("}");
            mTarget.print(indent2);

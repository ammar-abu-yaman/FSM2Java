...
//
    ...
    public SmcState(String name, int lineNumber, SmcMap map)
    {
        ...
        if (name.compareToIgnoreCase(DEFAULT_TRANSITION) == 0)
        {
    ...
    /**
     * Returns {@code true} if this state has a transition
     * defined which equals the given transition.
     * @param trans check if this transition is define in this
     * state.
     * @return state contains the defined transition.
     */
    public boolean hasTransition(final SmcTransition trans)
    {
        return (mTransitions.contains(trans));
    } // end of hasTransition(SmcTransition)

    /**
     * Returns {@code true} if this state has an explicit
     * {@code Default} transition defined and {@code false}
     * otherwise.
     * @return {@code true} if state has a {@code Default}
     * transition.
     */
    public boolean hasDefaultTransition()
    {
        Iterator<SmcTransition> transIt =
            mTransitions.iterator();
        SmcTransition transition;
        boolean retcode = false;

        while (transIt.hasNext() && !retcode)
        {
            transition = transIt.next();
            retcode =
                (transition.getName()).equals(DEFAULT_TRANSITION);
        }

        return (retcode);
    } // end of hasDefaultTransition()
    ...
    public boolean callDefault(final String name)
    {
        ...
        for (SmcTransition transition : mTransitions)
        {
            if (transition.getName().equals(DEFAULT_TRANSITION))
            {
                return false;
            }
        }
